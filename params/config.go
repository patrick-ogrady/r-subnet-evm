...
func (c *ChainConfig) String() string {
	...
	return fmt.Sprintf("{ChainID: %v Homestead: %v EIP150: %v EIP155: %v EIP158: %v Byzantium: %v Constantinople: %v Petersburg: %v Istanbul: %v, Muir Glacier: %v, Subnet EVM: %v, FeeConfig: %v, AllowFeeRecipients: %v, ContractDeployerAllowListConfig: %v, ContractNativeMinterConfig: %v, RandomPartyConfig: %v Engine: Dummy Consensus Engine}",
		c.ChainID,
		c.HomesteadBlock,
		c.EIP150Block,
		c.EIP155Block,
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, headHeight *big.Int, headTimestamp *big.Int) *ConfigCompatError {
	...
	// Check that the configuration of the optional stateful precompiles is compatible.
	if isForkIncompatible(c.RandomPartyConfig.Timestamp(), newcfg.RandomPartyConfig.Timestamp(), headTimestamp) {
		return newCompatError("RandomParty fork block timestamp", c.RandomPartyConfig.Timestamp(), newcfg.RandomPartyConfig.Timestamp())
	}
...
func (c *ChainConfig) enabledStatefulPrecompiles() []precompile.StatefulPrecompileConfig {
	...
	if c.RandomPartyConfig.Timestamp() != nil {
		statefulPrecompileConfigs = append(statefulPrecompileConfigs, &c.RandomPartyConfig)
	}
